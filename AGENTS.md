...
The quarantine mechanism itself is **not** retired: `[SkipOnLinuxCiFact]` still skips the affected `LiveCoreClrProcessTests` cases on Linux CI (they still run locally and on Windows), and `DOTNET_DBG_MCP_RUN_QUARANTINED_LINUX_TESTS=1` still forces them back on for manual investigation. Issue #685 tracks re-validating (un-quarantining) these tests once the fixed ClrMD version ships — do that validation there rather than reviving the retired workflows.
...
Docs-only probe #2 for #861 job-level skip validation (based on feature branch).
