...
After upgrading to ClrMD 4.1.745802, the strict Linux gate completed 20 clean sequential Core/MCP rounds (#685). The ten formerly quarantined tests now use ordinary `[Fact]` attributes; no override environment variable is needed. Ordinary CI runs every suite once via `scripts/ci-test.py`: nonzero command exits, incomplete discovery/TRX inventories, failed/aborted run outcomes, and missing evidence fail the job without retries. `scripts/clrmd-regression-tests.json` preserves the historical required-Passed inventory for both ordinary CI and the optional revalidation gate. Legitimate unrelated platform skips and the intentional CrashGuard target crash remain allowed in ordinary CI. See [`docs/clrmd-linux-revalidation.md`](./docs/clrmd-linux-revalidation.md) for evidence and limits; the concurrent aggregate-solution scenario in #879 is not established as resolved by sequential runs.
...
and truncating at the end. Collectors surface a cap hit via an explicit
`notes` entry naming the constant and the drop/eviction count. Check
[`docs/resource-boundedness.md`](./docs/resource-boundedness.md)
for each collector's exact retention and reporting behavior. See that doc for
