...
public class StdioTransportSmokeTests
{
    ...
    private static string? FindServerDll()
    {
        ...
        while (current is not null && !File.Exists(Path.Combine(current.FullName, "DotnetDiagnostics.slnx")))
        {
        ...
        var candidate = Path.Combine(current.FullName, "src", "DotnetDiagnostics.Mcp", "bin", cfg, tfm, "DotnetDiagnostics.Mcp.dll");
