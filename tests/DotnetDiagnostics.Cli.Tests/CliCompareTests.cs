...
public sealed class CliCompareTests : IDisposable
{
    ...
    public void TrySaveComparableSnapshot_ThreadPool_WritesScalarOnlySnapshotJson()
    {
        ...
        var snapshot = new ThreadPoolEventSnapshot(
            ...
            timestamp,
            TimeSpan.FromSeconds(5),
            [new ThreadPoolCountBucket(timestamp, 2, ThreadPoolEvidence.RuntimeObserved), new ThreadPoolCountBucket(timestamp.AddSeconds(1), 5, ThreadPoolEvidence.RuntimeObserved)],
            Array.Empty<ThreadPoolCountBucket>(),
            [new ThreadPoolHillClimbingSample(timestamp, "Starvation", 2, 5, 10, ThreadPoolEvidence.RuntimeObserved)],
