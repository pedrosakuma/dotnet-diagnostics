...
public sealed class ClrMdDumpInspector : IDumpInspector
{
    ...
    {
        var (typeStats, totalBytes, segments, delegateAgg, stringAgg, taskTimerAgg) = WalkHeap(runtime, opts, warnings, ct);
        ...
        return new RuntimeSummary(byBytes, byInstances, heapSummary, retention, roots, finalizable, segments, statics, delegates, duplicates, gcHandles, asyncOperations, timers);
    }
    ...
    private static (Dictionary<TypeKey, RawTypeStat> Stats, long TotalBytes, IReadOnlyList<SegmentStat> Segments, Dictionary<DelegateKey, RawDelegateStat>? Delegates, Dictionary<string, RawStringStat>? Strings, RawTaskTimerAggregation TaskTimers) WalkHeap(
        ClrRuntime runtime, DumpInspectionOptions opts, List<string> warnings, CancellationToken ct)
    {
        ...
        return (stats, total, segmentStats, delegates, strings, taskTimers);
    }
