...
/// <c>query_snapshot</c>: the tool answers narrow, parameterized questions; this Resource
...
public sealed class HeapSnapshotResources
{
        ...
        string handle)
    {
        ...
        if (snapshot is null)
        {
            return JsonSerializer.Serialize(
                new HeapSnapshotErrorPayload(
                    Kind: "unknown",
                    Error: $"Handle '{handle}' is unknown or expired. Re-run inspect_heap to issue a fresh handle."),
        ...
        // Callers that need previews must go through `query_snapshot` which enforces
