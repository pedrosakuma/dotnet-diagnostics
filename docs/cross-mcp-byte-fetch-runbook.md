...
2. It calls `get_module_bytes(moduleVersionId="…", asset="pe", processId=12345)` and keeps following the returned `NextActionHint` until `nextOffset == null`.
...
The same pattern works for dumps: `collect_process_dump(confirm=true)` → `get_dump_bytes(dumpFilePath, offset, maxBytes)` → materialize locally → hand off to the dump-aware sibling MCP.
...
- Both `get_module_bytes` and `get_dump_bytes` require the literal scope **`module-bytes-read`**.
