# Core SQLite capture storage contract — format 1
...
record, index, writer, and reader versions. Each is currently `1`. The only
required feature is `normalized-scalars-v1`; unknown/missing versions or
required features fail closed. Format 1 is the only supported format:
pre-freeze experimental databases are not imported. Future schema changes
require an explicit evolution decision and independent fixtures. There is no
claimed previous-version support before a previous frozen format exists, and
no in-place sealed migration.
