# Core SQLite capture storage contract — ordinary v2, imported v3, previous v1
...
Portable ZIP transport and untrusted import follow a separate approved contract:
[`portable-capture-contract.md`](portable-capture-contract.md) (#1048). It specifies
independent multi-capture bundles, bounded admission and package-3 import-derived
provenance. Ordinary collection remains package 2; executing reader 3 also reads
imported package 3 and previous package 1. Trusted-root checks are not sufficient
...
The explicitly configured [Core importer](portable-capture-import.md) uses
separate contained source admission and trusted-schema rebuild processes.
...
## Independent format axes and v1/v2/v3 compatibility
...
| Axis | Frozen previous package | Ordinary package | Imported package |
|---|---:|---:|---:|
| Package / canonical artifact metadata | 1 | 2 | 3 |
| SQLite schema | 1 | 1 | 1 |
| Occurrence record representation | 1 | 1 | 1 |
| Required indexes | 1 | 1 | 1 |
| Producing writer generation | 1 | 2 | 3 |
| Minimum required reader generation | 1 | 2 | 3 |
...
v3 additionally requires `portable-source-v1`, carrying bounded permanent
origin, immediate source and artifact mappings. Ordinary collection does not
manufacture import provenance or silently upgrade sealed packages.
...
`CaptureReaderIdentity("DotnetDiagnostics.Core.Captures.CaptureReader", 3)`.
Thus opening a v1 package reports source required-reader 1 and actual executing
reader 3; stored metadata is never relabeled as the executing implementation.
There is no in-place sealed migration, reindex, checkpoint, or evidence rewrite
when opening any supported version.
