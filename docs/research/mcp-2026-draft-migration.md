...
- **RC.1 breaking changes are not applicable to this repo:** all four breaking changes in `2.0.0-rc.1` target either the Tasks extension API or the SDK's client-side OAuth flows. This repo is a server only and uses its own custom bearer/OIDC middleware; none of the OAuth changes apply.
...
| Harden OAuth issuer validation — `AuthorizationRedirectDelegate` → `AuthorizationCallbackHandler` | No | This repo is a **server** using custom bearer/OIDC middleware (`BearerTokenMiddleware`, `OidcJwtProvider`). It does not use any SDK client-side OAuth flows; the `AuthorizationRedirectDelegate` API is part of the MCP **client** OAuth machinery. |
