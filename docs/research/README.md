...
|---|---|---|
| [`icu-symbol-feasibility.md`](./icu-symbol-feasibility.md) | [#987](https://github.com/pedrosakuma/dotnet-diagnostics/issues/987) | Exact-image Windows ICU public-symbol availability, age/range counterexamples and bounded offline preflight; historical sampled coverage and actual DIA lookup remain unestablished |
...
| [`multi-version-target-support.md`](./multi-version-target-support.md) | [#884](https://github.com/pedrosakuma/dotnet-diagnostics/issues/884) | Whether target processes on older .NET (6/7/8/9) can be diagnosed today — empirically confirmed working end-to-end on .NET 6, 8, and 9 with zero code changes; **acted on**: `CrossVersionTargetTests` + `samples/MultiVersionSample` now keep .NET 8/9 continuously validated in CI, formalized in [`../runtime-version-compat-matrix.md`](../runtime-version-compat-matrix.md) |
| [`gcdump-known-graph-validation.md`](./gcdump-known-graph-validation.md) | [#928](https://github.com/pedrosakuma/dotnet-diagnostics/issues/928) | Bounded test-only known-graph experiment separating raw node/edge/root observations, production type projections, timeout aborts, pressure timing, and unobservable EventPipe loss |
