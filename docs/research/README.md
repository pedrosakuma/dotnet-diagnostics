...
| [`multi-version-target-support.md`](./multi-version-target-support.md) | [#884](https://github.com/pedrosakuma/dotnet-diagnostics/issues/884) | Whether target processes on older .NET (6/7/8/9) can be diagnosed today — empirically confirmed GO for .NET 8+ with zero code changes; real gap is missing CI/sample coverage, not a technical block |
