...
- immutable schema version, build identities, capture time, runner class,
  runtime, OS, RID, architecture, and GC mode;
...
runner class, workload contract, and parameters must match. Hosted and dedicated
runners are different environments even when their runtime versions match.
