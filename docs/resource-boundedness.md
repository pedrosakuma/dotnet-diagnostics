...
| `EventPipeGcCollector` (`Gc/`) | `maxEvents` (default 200) independently caps raw `GcEvent` rows and `GCHeapStats` samples | Exact `TotalCollections`, `TotalPauseTime`, `MaxPauseTime`, and per-generation counts continue updating for every paired GC start/stop; the first `maxEvents` raw event rows and heap-stat samples are retained | Later raw event and heap-stat detail rows only; aggregate memory stays constant after the cap | `DroppedEvents` / `DroppedHeapStats`; GC query views expose retained/dropped counts. Aggregate and per-generation counts remain exact, while histogram/timeline/longest-pause/by-generation pause-detail views describe the retained prefix |
