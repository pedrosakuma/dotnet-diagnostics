...
| `EventPipeActivityCollector` (`Activities/`) | `maxActivities` (caller-supplied, default 200) | The first `maxActivities` completed Activity stop events, sorted deterministically after collection | Later completed activities; exact `TotalActivities`/`CompletedActivities` continue counting observed stop events | `TotalActivities > Activities.Count`; collector summary and `query_snapshot(view="trace")` emit explicit retention/incompleteness warnings |
