...
| `collect_batch` correlated counter projection (`Mcp/Tools/CollectBatchSalientEvidence`) | `MaxInlineCounters = 18`; at most two fixed explanatory notes and one fixed-size `gen2Evidence` record | The normal headline counter set; when paired GC reports Gen2 activity, also `gen-2-size`, `loh-size`, and `gc-fragmentation` when emitted | Every other counter stays only in the existing full counter artifact behind the handle; no second full table is materialized | Inline note names the cap and full captured count; `gen2Evidence` labels interval-delta, rate, process-cumulative, and collector-window scopes separately |
