...
| Perf/ETW off-CPU & CPU samplers (`OffCpu/`, `CpuSampling/`) | `perf record --max-size` (already capped pre-fix; Linux off-CPU now uses independent caps: 128 MiB for sched_switch DWARF and 64 MiB for the optional target-scoped raw-syscall companion) + the parser/aggregator streams `perf script`/ETW output instead of buffering the full text/span list before aggregating | Same final aggregate (call tree / stack rollups) as before for CPU and sched off-CPU. Off-CPU syscall labels are best-effort enrichment. | Base stack rows are not dropped by syscall failure/truncation; if the optional syscall companion fails or caps, affected spans simply have no syscall label | Off-CPU notes explicitly name sched/raw perf.data cap hits; syscall companion failures say stacks were returned without syscall labels |
