...
**Most** of these fixes surface the cap hit in the result (`Notes`/similar
fields carry an explicit message with the cap and the drop/eviction count) —
but this isn't universal: `RequestsNowCollector`'s snapshot-queue drops are
only logged at debug level (no result note), so don't assume every collector
below reports truncation identically. If you are diagnosing a workload that
plausibly exceeds these caps (e.g. thousands of distinct SQL shapes, a
sustained request flood), read the `Notes` on the returned snapshot — and, for
`requests-now`, check the server/CLI debug logs too — before trusting the
numbers as exhaustive.
