...
|---|---|---|---|---|
| 1 | "CPU pegged at 100% on one core" | `GET /cpu-burn?ms=3000` | `inspect_process(view="triage")`, `collect_sample(kind="cpu")` | triage retains host-normalized `cpu-usage` and estimates effective cores, so `cpu.effective-core-consumption` is salient even on a many-core host; sampling identifies the hot frames |
...
| 9 | "A trivial in-memory lookup pegs one or more cores" | sustained `GET /culture-lookup?iterations=3000000` | `inspect_process(view="triage")`, `collect_sample(kind="cpu")` → `query_snapshot(view="call-tree")` | triage reports both host-normalized CPU and effective-core consumption, then emits the diagnosis-agnostic `cpu.compute-demand`; the call-tree drill, not triage, attributes exclusive runtime cost |
