...
claims. `--json` exposes the same rule as the machine-readable
`data.untrustedDataBoundary` object while preserving the captured strings
(subject only to documented sensitive-data redaction and byte caps).
