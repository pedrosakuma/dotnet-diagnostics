...
[^heap]: ClrMD's DAC has no NativeAOT implementation; there is no public design for one upstream. See **Honest non-goals** below. This footnote covers the **ClrMD-backed** `source="live"`/`source="dump"` heap walks; the EventPipe `source="gcdump"` path is separately unavailable on NativeAOT (see `[^aot-gcdump]`).
[^aot-gcdump]: **Not supported on NativeAOT — requesting a gcdump crashes the target.** gcdump drives the runtime-emitted `GCHeapSnapshot` EventPipe keyword (`0x1980001`). On NativeAOT .NET 10 (reproduced on SDK 10.0.201), enabling that keyword **terminates the target process**: the runtime segfaults (`Segmentation fault (core dumped)`) mid-handshake and the diagnostic IPC stream closes (`EndOfStreamException`). The benign GC keyword (`0x1`) is safe but emits no `GCBulkNode` object-graph events. Because attempting a gcdump would kill a production target, the capability is deliberately **withheld** on NativeAOT: `inspect_process` reports `canCollectGcDump: false`, and both the use-case and the collector refuse with a `NotSupported` error before opening a session. Root-cause research (issue [#467](https://github.com/pedrosakuma/dotnet-diagnostics/issues/467)) found the object-graph stream is *address-only* even in principle (NativeAOT emits empty `BulkType` names, and `WalkStaticsAndCOMForETW` is a no-op stub so static-only-rooted objects appear unrooted). That spike's GO verdict was empirically reversed once implementation issue [#471](https://github.com/pedrosakuma/dotnet-diagnostics/issues/471) reproduced the crash above — so the fidelity ceiling is moot while the dump itself crashes the runtime. A follow-up spike, [#474](https://github.com/pedrosakuma/dotnet-diagnostics/issues/474), considered enabling `GCBulkRootStaticVar` upstream to close the static-roots gap. **Use `collect_process_dump` for a native dump instead.** Tracked in [#471](https://github.com/pedrosakuma/dotnet-diagnostics/issues/471).
